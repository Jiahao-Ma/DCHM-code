...
def get_mono_depth(imgs):
    ...
    disps = []
    for image in imgs:
        imagePIL = Image.fromarray(image.astype(np.uint8))
        w, h = imagePIL.size
        disp = depth_model(imagePIL)['predicted_depth']
        disp = F.interpolate(disp[:, None], (h, w), mode="bilinear", align_corners=True)[0, 0]
        disps.append(disp.unsqueeze(-1).cuda())
    return torch.stack(disps)
    
def depth_rank_loss(pred_depths, gt_depths, masks=None, sample_n=512):
    loss = 0
    for pred_depth, gt_depth, mask in zip(pred_depths, gt_depths, masks):
        pred_depth, gt_depth = pred_depth[mask].view(-1), gt_depth[mask].view(-1)
        ...
        # note that we use inverse mono depth
        ...
        loss += torch.mean(torch.clamp(depth_loss0, min=0.0))
    return loss
...
def step02_train(
         ...
         dataset = None,
         save_folder = None,
         using_mono_depth=True
         ):
    if using_mono_depth and train_args.loss_rank_depth_w > 0:
        images = data['nonnorm_image'].squeeze(0).permute(0, 2, 3, 1).cpu().numpy()
        mono_depths = get_mono_depth(images)
    ...
    data_folder = os.path.join(data_root, 'depths', frame_idx)
    super_pixel_ply = os.path.join(data_folder, f'init_gs_{round_name}.ply')
    ...
    strategy = DefaultStrategy(refine_start_iter=100, refine_stop_iter=train_args.epochs, reset_every=train_args.epochs//4, refine_every=300, verbose=True)
    ...
    if round_idx > 1:
        depth_paths = os.path.join(data_folder, f'depth_{round_idx-1}_2.npy')
        depth_mask_paths = os.path.join(data_folder, f'mask_{round_idx-1}_2.npy')
    ...
    for epoch in pbar:
        ...
        renders, pred_alphas, info = rasterize_splats(splats, w2cs, Ks, width, height)
        ...
        if loss_global_depth_w > 0 and gt_depth is not None:
        ...
        if loss_rank_depth_w > 0:
            loss_rank_depth = depth_rank_loss(pred_metric_depth, mono_depths, gt_mask_comb_b)
        else:
        ...
        if epoch < train_args.sparsity_epoch:
            loss = loss_rgb * loss_rgb_w + loss_bg * loss_bg_w + loss_quantization * loss_quantization_w + loss_local_depth * loss_local_depth_w + loss_global_depth * loss_global_depth_w + loss_rank_depth * loss_rank_depth_w    
        else:
            loss_sparsity = torch.mean(pred_opacity)
            loss = loss_rgb * loss_rgb_w + loss_bg * loss_bg_w + loss_local_depth * loss_local_depth_w + loss_quantization * loss_quantization_w + loss_sparsity * loss_sparsity_w + loss_global_depth * loss_global_depth_w + loss_rank_depth * loss_rank_depth_w
        ...
        if epoch < train_args.sparsity_epoch:
            pbar.set_description(f'Epoch {epoch} Loss local dc: {local_depth_loss_print:6.6f} | Loss global dc: {global_depth_loss_print:6.6f} | Loss rank_d: {rank_depth_loss_print:6.6f} | Loss bg: {bg_loss_print:6.6f} | Loss rgb: {rgb_loss_print:6.6f} | Loss quant: {quantization_loss_print:6.6f} | Loss: {np.mean(losses):.6f}')
            ...
            pbar.set_description(f'Epoch {epoch} Loss local dc: {local_depth_loss_print:6.6f} | Loss global dc: {global_depth_loss_print:6.6f} | Loss rank_d: {rank_depth_loss_print:6.6f} | Loss bg: {bg_loss_print:6.6f} | Loss rgb: {rgb_loss_print:6.6f} | Loss quant: {quantization_loss_print:6.6f} | Loss sparsity: {sparsity_loss_print:6.6f} | Loss: {np.mean(losses):.6f}')
...
def step03_generate_matching_label(ckpt_path, data, superpixel_data, num_cam = 7, depth_offset_threshold=100, bg_filter=True, depth_filter=True, round_name='1_1'):
    ...
    splats["opacities"] = torch.ones_like(splats["opacities"]) * 20
    w2cs = torch.cat([cam['w2c'] for cam in data['novel_view']], dim=0).float().cuda()
    Ks = torch.cat([cam['K'] for cam in data['novel_view']], dim=0).float().cuda()
    width = data['novel_view'][0]['width'].item()
    height = data['novel_view'][0]['height'].item()
...
def main():
    ...
    args_src = [
                ...
                '--loss_bg_w', '1.0',
                '--loss_rgb_w', '1.0',
                '--loss_local_depth_w', '0',#'1e-5',#'1e-6',
    ...
    parser.add_argument("--loss_rank_depth_w", type=float, default=1e-4, help="the weight of rank depth loss")
    ...
    print('Start index: {} | End index: {} | {} data in total'.format(start_with, end_with, len(wildtrack_data)))
    for idx, data in enumerate(wildtrack_dataloader):
        ...
        empty_cache()
        cur_t = time.time()
        model, trained_gs_path = step02_train(args, frame_idx, data_root, data, all_images, round_name=args.round, remove_init_gs= True if args.round == '1_1' else False, dataset=wildtrack_data) #, save_folder='output/gs_grow_prune') FOR DEBUG
